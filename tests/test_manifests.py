...
class TestNotionManifest:
    manifest = load_manifest(PROJECT_ROOT / "notion-module")

    def test_manifest_version(self):
        assert self.manifest["manifest_version"] == 2

    def test_id(self):
        assert self.manifest["id"] == "notion"

    def test_name(self):
        assert self.manifest["name"] == "Notion Airlock"
    ...
    def test_auth_fields(self):
        ...
        assert auth["vault_provider"] == "notion"
        assert auth["field"] == "NOTION_API_KEY"

    def test_requires_network(self):
        assert "api.notion.com" in self.manifest["requires"]["network"]

    def test_requires_no_subprocess(self):
        assert self.manifest["requires"]["subprocess"] is False

    def test_command_count(self):
        assert len(self.manifest["commands"]) == 8
    ...
    def test_all_commands_require_key(self):
        for cmd in self.manifest["commands"]:
            assert "NOTION_API_KEY" in cmd["requires"]
    ...
    def test_function_names_match(self):
        handler = get_handler(PROJECT_ROOT / "notion-module")
        for cmd in self.manifest["commands"]:
...
class TestDropboxManifest:
    manifest = load_manifest(PROJECT_ROOT / "dropbox")
